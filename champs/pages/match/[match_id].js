    ...
    <>
      ...
      {timeline?.frames &&
        timeline.frames.map((frame) => {
          ...
          return (
            <div>
              <div className="timestamp">{msToTime(frame.timestamp)}</div>
              <div className="personFramescontainer">
                ...
                {personFrames &&
                  Object.values(personFrames).map((item) => {
                      ...
                      <>
                        <div className="item">
                          ...
                          <div className="champion">
                            {participants &&
                              participants[item?.participantId]?.champName}
                          </div>
                          ...
                          <div className="jungle-cs">
                            {item?.jungleMinionsKilled}
                          </div>
                          <div className="position">{position}</div>
                        </div>
                      </>
...
export async function getServerSideProps({ params }) {
  ...
  let champions = null;
  if (match_id) {
    match = await getFromDatabase("matches/getmatch/" + match_id);
    timeline = await getFromDatabase("timeline/" + match_id);
    champions = await getFromDatabase("champions");
  }
  return {
